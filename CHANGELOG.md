...
### 추가
- **v4 워크포워드 그리드 지원 + --max-hold CLI 옵션** — `StrategyParams`에 `donchian_entry_period`/`donchian_exit_period` 필드 추가. `_INT_GRID_FIELDS` 상수로 `parse_grid` int 변환 일반화. `_params_to_strategy` v4 분기. `backtest`/`walkforward` 커맨드에 `--max-hold`(기본 24 유지) 옵션 배선. walkforward v4 + V1 기본 그리드 조합 시 오류 가드. HTML 보고서 템플릿 donchian 파라미터 조건부 렌더링 추가. 결과: `reports/walkforward/v4_summary.md`
