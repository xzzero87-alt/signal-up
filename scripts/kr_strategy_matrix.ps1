...
if ($Quick) {
    ...
    $OutName = "kr_strategy_matrix_quick.csv"
} else {
    $Names   = $Full
    $OutName = "kr_strategy_matrix.csv"
}
...
foreach ($n in $Names) {
    # 시작월(2022-01)을 콕 집어 확인 — 부분 수집(예: 2024~만 받힌 종목) 시 빈 구간 방지
    $probe = "data\candles\$($n.Code)\1440\2022-01.parquet"
    if (Test-Path $probe) {
        ...
        foreach ($attempt in 1..3) {
            ...
            uv run signal fetch-candles-kr --market $n.Code --from 2022-01-01 | Out-Null
...
foreach ($n in $Names) {
    foreach ($p in $Periods) {
        ...
        $raw = uv run signal backtest --market $n.Code --from $p.From --to $p.To `
                   --strategy $Strategy --timeframe $Timeframe --max-hold $MaxHold 2>&1 |
